...
async function formFiller(page, data) {
  for (const { label, value } of data) {
    ...
    switch (type) {
      ...
      case 'radio':
        await page.check(
          `${dataPath}[${xPathContainsIgnoreCase(
            'data-label-option',
            value
          )}]//input`
        );
        break;
      case 'checkbox':
        const checkboxXPath = `${dataPath}//input`;
        value
          ? await page.check(checkboxXPath)
          : await page.uncheck(checkboxXPath);
        break;
      case 'single-select':
        // open dropdown
      ...
      case 'multi-select':
        ...
        const doneButtonSelector = `//button[normalize-space(.)='Done']`;
        (await page.$(doneButtonSelector)) &&
